...
class AmplitudeEventManager {
  ...
  Future<void> actionEvent(String viewName, String actionName, [dynamic? property, dynamic? property2, Map<String, dynamic>? additionalInfo]) async {
    if (actionName == 'enddetection') {
      ...
      await Amplitude.getInstance().logEvent('action_${viewName}_$actionName', eventProperties: {'settedTime': property, 'alarmCount': property2, ...?additionalInfo});
      return;
    }
    else if (actionName == 'set_goal') {
      await Amplitude.getInstance().logEvent('action_${viewName}_$actionName', eventProperties: {'goaltype': property, 'setted_value': property2});
    }
