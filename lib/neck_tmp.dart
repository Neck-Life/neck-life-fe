...
import 'package:mocksum_flutter/util/airpods/RotationDisplay.dart';
...
class NeckState extends State<Neck> with SingleTickerProviderStateMixin {
  ...
  List<double> positions = [10.0];

  //각도측정 & 위치측정
  RotationDisplay rotate = RotationDisplay();
  PositionDisplay pose = PositionDisplay();
  ...
  void _startAirpodSensing() {
    setState(() {
      _subscription = FlutterAirpods.getAirPodsDeviceMotionUpdates.listen((data) {
        ...
        _pitch = rotate.getPitch();
        rotate.process(data);
        rotate.activePushAlarm(_showPushAlarm);

        _rotateDeg = pose.getPosition(1);
        pose.processSensorData(data);
        ...
        builder: (context, child) {
          return Container(
              ...
              decoration: ShapeDecoration(
                color: rotate.isTurtleNeck() ? const Color(0xFFFFC2B9) : const Color(0xFFBBFFB9),
